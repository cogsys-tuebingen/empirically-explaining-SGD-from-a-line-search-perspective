...
class SGDLineSamplerOnPosition(Optimizer):
    """ Performs SGD training. For each update the full-batch loss along the line, each possible update step direction originating from different batches, is measured"""
    ...
    def __init__(self, model=required, lr=0.1, lr_milestones=[], momentum=0.0, l2_reg=0.0,
                 ...
                 train_dataset=required, train_dataset2=required, train_data_set_size=required,
                 validation_dataset=required,
                 validation_data_set_size=required, batch_size=required, sample_interval=0.5, resolution=0.006,
                 max_interval_enlargements=5, sample_step_interval=100):
        ...
        self.loss_with_mean = torch.nn.CrossEntropyLoss()
        ...
        super(SGDLineSamplerOnPosition, self).__init__(params, defaults)
        ...
        def deterministic_loss():
            with self.random_seed_torch(int(seed)):
                ...
                losses = self.loss(logit,
                                   y.cuda())  # better get all losses at once independent of the batchsize
                return losses

        return deterministic_loss

    def measure_exact_loss(self, directin_batch_loss_fn, position_line_number):
        ...
        with torch.no_grad():
            ...
            for remeasure in range(max_interval_enlargements):
                ...
                direction_batch_losses = np.zeros((len(sorted_measure_locations)))
                ...
                for i, location in enumerate(sorted_measure_locations):
                    ...
                    direction_batch_loss = directin_batch_loss_fn()
                    direction_batch_losses[i] = direction_batch_loss
            ...
            path = self.line_data_save_dir + "/line_data_{}_{}.pickle".format(self.performed_training_steps,
                                                                              position_line_number)
            with open(path, 'wb') as f:
                pickle.dump((train_line_losses, val_line_losses, all_measure_locations, direction_batch_losses,
                             self.update_step_size, self.direction_norm, self.is_extrapolation, position_line_number),
                            f)

            zero_index = np.where(all_measure_locations == 0)[0][0]
            gradients = np.gradient(direction_batch_losses[zero_index - 1:zero_index + 2],
                                    all_measure_locations[zero_index - 1:zero_index + 2])

            aa = gradients[1]
            bb = self.direction_norm
            print("direction_norm {}  approximated directional derivative {}".format(bb, aa))
            ...
            loss_sgd_step_index = self.find_nearest(all_measure_locations, sgd_update_step_location)
            loss_sgd_step = mean_train_loss[loss_sgd_step_index]

            near_0_index = self.find_nearest(all_measure_locations, 0.0)  # should be inside but just to be save
            self._log_print("step_number: ", self.performed_training_steps, "position_line_number ",
                            position_line_number)
            ...
            if self.is_plot:
                ...
                path = self.line_data_save_dir + "/line_{}_{}.png".format(self.performed_training_steps,
                                                                          position_line_number)
                plt.savefig(path)
                plt.close()

            self._perform_sgd_update(self.params, -self.current_position)
    ...
    def save_last_direction(self, params):
        with torch.no_grad():
            for p in params:
                if p.grad is None:
                    continue
                param_state = self.state[p]
                if 'dir_buffer_backup' not in param_state:
                    buf = param_state['dir_buffer_backup'] = torch.zeros_like(p.grad.data, device=p.device)
                else:
                    param_state['dir_buffer_backup'] = param_state['dir_buffer'].clone()
            self.norm_backup = self.direction_norm.clone()

    def load_last_direction(self, params):
        with torch.no_grad():
            for p in params:
                if p.grad is None:
                    continue
                param_state = self.state[p]
                param_state['dir_buffer'] = param_state['dir_buffer_backup'].clone()
            self.direction_norm = self.norm_backup.clone()
    ...
    def step(self, loss_fn):
        ...
        with torch.no_grad():
            ...
            self._perform_update_step(params, 0, loss_fn, new_direction=True)
            self.update_step_size = (self.sgd_lr * self.direction_norm).item()
            if (self.performed_training_steps % self.sample_step_interval) == 0:
                print("measuring {} possible update step directions at step {}".format(self.steps_per_train_epoch,
                                                                                       self.performed_training_steps, ))
                self.save_last_direction(self.params)
                for s in range(self.steps_per_train_epoch):
                    self.zero_grad()
                    (x, y) = next(self.train_dataset2)

                    def loss_function():
                        logit = self.model(x.cuda())
                        loss = self.loss_with_mean(logit,
                                                   y.cuda())
                        return loss

                    self._perform_update_step(params, 0, loss_function, new_direction=True)

                    _, _ = self.measure_exact_loss(loss_function, s)
                    self.load_last_direction(self.params)
            step_size = self.update_step_size - self.current_position
            assert self.current_position == 0
            batch_loss = self._perform_update_step(params, step_size, loss_fn, new_direction=False)
    ...
    def save_checkpoint(self, step, dir):
        path = os.path.join(dir, "step_{0}.cpt".format(step))
        with open(path, 'wb') as f:
            torch.save({
                'step': step,
